...
{
    private static readonly Dictionary<string, string> BinaryToHexTable = new()
    {
    ...
    private static readonly Dictionary<string, string> HexToBinaryTable = new()
    {
    ...
    public static string BinaryToString(string value)
    {
        if (string.IsNullOrEmpty(value) || value.Length % 8 != 0)
            throw new ArgumentException("Binary string is not valid.");
    ...
    public static string HexToBinary(string value)
    {
        ...
        foreach (var c in value.ToLower()) binary.Append(HexToBinaryTable[c.ToString()]);
    ...
    {
        if (bytes == null || bytes.Length == 0) throw new ArgumentException("Byte array cannot be null or empty.");
    ...
    {
        return PerformBitwiseOperation(binary1, binary2, (b1, b2) => (b1 & b2).ToString());
    }
    ...
    {
        return string.Concat(binary.Select(b => b == '0' ? '1' : '0'));
    }
    ...
    {
        return binary.PadRight(binary.Length + shift, '0').Substring(0, binary.Length);
    }
    ...
    {
        return binary.PadLeft(binary.Length + shift, '0').Substring(shift);
    }
